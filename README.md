...
| 项目 | 说明 |
|---|---|
| 安装方式 | 双击运行，选「仅为我安装」即可（**不需要管理员权限**） |
| 系统要求 | Windows 7 SP1 及以上（64 位） |
| 卸载 | 开始菜单 → CS2 雌小鬼军需 → 卸载 |
| 快捷键 | **F1** 打开「关于」窗口 |

> 📌 **本程序由 HFUT2026_CKX 免费分享，禁止倒卖收费。**
> 如果你为它付过钱 —— 那你被骗了。

> ⚠️ **关于饰品图片**：安装包内的饰品贴图著作权属于 **Valve Corporation**，
> 仅供个人学习研究，**请勿商用、请勿再分发**。本项目与 Valve 无任何关联。
...
- 目的是劝诫赌博行为 —— 抽得越多亏得越多，程序里也会这么提醒你（真的）。
...
### 分发水印：`HFUT2026_CKX免费分享`

写在这些地方，抹掉需要改动多处：

| 位置 | 说明 |
|---|---|
| 程序集元数据 | 「属性 → 详细信息」里的产品名、文件说明、版权、公司 |
| 窗口标题 | `CS2 雌小鬼军需 ｜ HFUT2026_CKX免费分享` |
| 界面大标题 | 主界面顶部 |
| 关于对话框 | **按 F1** 打开，也可用 `--about` 参数启动 |
| **数据加密密钥** | 密钥由该字符串派生 —— 见下 |

### 数据文件加密

`data/` 下的 `items.json`、`prices.json`、`excluded.json`、`quiz.txt`
在分发版本中是加密的：

```
明文前 4 字节 = "CKXE" 魔数，其余为 XOR 密文
密钥 = FNV-1a 扩散自 "HFUT2026_CKX免费分享|cs2-mesugaki-quartermaster|v1"
```

设计要点：

- **读取时自动识别**：有魔数就解密，没有就按明文读
  → 开发时直接改明文 JSON，不用每次加密
- **水印与数据绑定**：改掉水印就解不开数据，所以删不掉
- **图片不加密**：PNG 本身无保护价值，且 160MB 解密有性能开销
- 加密/解密实现在 `src/Crypto.cs`，加密脚本是 `tools/encrypt_data.py`

> 诚实说明：这是**简单的异或加密**，能挡住直接改 JSON 的人，
> 但挡不住有逆向经验的人。要真正保护 .NET 程序需要混淆器（如 ConfuserEx）。

### 构建时的自动加密

`build.ps1` 在部署数据后会调用 `tools/encrypt_data.py`，
因此每次构建产出的都是加密数据，并自动清理 `*.plain` 明文备份。
...
- **窗口最小化守卫**：最小化时 `ClientSize` 会变成 `0×0`，
  而 GDI+ 的 `LinearGradientBrush` 对零尺寸矩形会抛 `ArgumentException`，
  因此所有跟随窗口尺寸的自绘控件都在 `OnPaint` 开头做了尺寸判断
